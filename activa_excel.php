...
header("Content-Disposition: attachment; filename=Activaciones_".date('d-m-Y h:i a',time() - 3600*date('I')).".xls");
...
$cant_clie_dist = mssql_query("select distinct safact.codclie FROM safact inner join saclie on safact.codclie = saclie.codclie  where DATEADD(dd, 0, DATEDIFF(dd, 0, safact.FechaE)) between '$fechai2' and '$fechaf2' and safact.codvend = '$convend' and tipofac = 'A'");
$num_cant_clie_dis = mssql_num_rows($cant_clie_dist); 
?>
<?php if ($aux == 0){ ?>
<p data-role="header" data-theme="b"><h3>Clientes NO ACTIVADOS , EDV: <?php echo $convend; ?>, Desde: <?php echo $_GET['fechai']; ?> Hasta:  <?php echo $_GET['fechaf']; ?></h3></p>	
<table width="622" height="78" border="0" data-role="table" id="table-column-toggle" data-mode="columntoggle" class="ui-responsive table-stroke">
  ...
  <?php 
			...
			$busca_no_activado = mssql_query("select distinct saclie.codclie FROM saclie inner join safact on saclie.codclie = safact.codclie where DATEADD(dd, 0, DATEDIFF(dd, 0, safact.FechaE)) between '$fechai2' and '$fechaf2' and tipofac = 'A' and saclie.codvend = '$convend' and safact.codclie = '$codecliente'");
...
<?php }else{ ?>
<p data-role="header" data-theme="b"><h3>Clientes ACTIVADOS , EDV: <?php echo $convend; ?></h3></p>	
<table width="622" height="78" border="0" data-role="table" id="table-column-toggle" data-mode="columntoggle" class="ui-responsive table-stroke">
  <thead class="ui-btn-active">
	<tr bgcolor="#0099FF">
	<th data-priority="2"><div align="center"><strong>CodClie</strong></div></th>
